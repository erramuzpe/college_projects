...
lista_d_var:	{}//puede ser el vacio
		| lista_id tok_dos_puntos tipo tok_punto_y_coma lista_d_var {}
...
exp:		exp tok_mas exp {
				      nodo *T = newtemp();
				      $$.place = T;
				      modificarTipo(&t,ENTERO,T->nodoTS->id);
				      gen(SUMA,$1.place,$3.place,T);
				      $$.tipo = ENTERO;
				}		  
		| exp tok_menos exp {
				      nodo *T = newtemp();
