...
% Asier Erramuzpe Practica 2 Gradiente
...
function salida = gradiente(imagen, c, alpha, tol)
	...
	while (errorini-errorfin >= tol)
		...
		for i=1:c
			...
			for x=1:fila
				...
				for y=1:col
					%Txungo
                    suma = suma + (R(x,y) - max(min(Aant(:, x), Bant(:, y)))) * (min(Aant(i,x), Bant(i,y)) >= max(min(Aant([1:i-1 i+1:c], x), Bant([1:i-1 i+1:c], y)))) * (Aant(i,x) <= Bant(i,y));
				end
		...
		for i=1:c
			...
			for y=1:col
				...
				for x=1:fila
					%Txungo
                    suma = suma + (R(x,y) - max(min(Aant(:, x), Bant(:, y)))) * (min(Aant(i,x), Bant(i,y)) >= max(min(Aant([1:i-1 i+1:c], x), Bant([1:i-1 i+1:c], y)))) * (Bant(i,y) <= Aant(i,x));
				end
	...
	imwrite(uint8(Rgor.*255),strcat('graditer',num2str(j),'c',num2str(c),imagen));
    toc
