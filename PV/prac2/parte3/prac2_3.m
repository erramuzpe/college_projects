...
for i=1:numsegmento-1
    ...
    % segmento sonoro   
        ...
        z=1;
        while (z<=120)
            u(z)=1; %tren de impulsos separados por T muestras. 
            L=L+1; % contador de nº de impulsos
            z=z+sordosonoro(i); % T es el periodo del pitch del segmento
        end
        G=sqrt(Ee(i)/L);
        if (exist('Gusimplificada')==0)
            Gusimplificada=G*u;
